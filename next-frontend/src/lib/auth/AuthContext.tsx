...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      if (error) {
        ...
        setRoles(
          userRoles?.map((ur) => ({
            ...
            permissions: ur.roles.permissions as Record<string, any>,
            is_system_role: ur.roles.is_system_role ?? false,
          })) || []
        );
      }
    };
  ...
  const hasPermission = (module: string, action: string, subAction?: string): boolean => {
    return roles.some((role) => {
      if (role.permissions['all_modules'] === true) return true;
      const modulePermissions = role.permissions[module];
      if (!modulePermissions) return false;
      if (subAction) {
        return modulePermissions[action]?.includes(subAction) || false;
      }
